...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 03-04-2025 a las 03:08:58
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DesbloquearUsuario` (IN `usuario_id` INT)   BEGIN
    UPDATE usuarios SET intentos_fallidos = 0, bloqueado = FALSE WHERE id = usuario_id;
END$$
...
ALTER TABLE `productos`
  ...
  ADD KEY `idx_pedidos_stock` (`stock`);
...
ALTER TABLE `ventas`
  ...
  ADD KEY `id_empleado` (`id_empleado`),
  ADD KEY `idx_pedidos_fecha` (`fecha_venta`);
...
--
ALTER TABLE `auditoria_pedidos`
  ADD CONSTRAINT `auditoria_pedidos_ibfk_1` FOREIGN KEY (`id_pedido`) REFERENCES `ventas` (`id_venta`) ON DELETE CASCADE;
...
--
ALTER TABLE `historial_pedidos`
  ADD CONSTRAINT `historial_pedidos_ibfk_1` FOREIGN KEY (`id_pedido`) REFERENCES `ventas` (`id_venta`) ON DELETE CASCADE;
